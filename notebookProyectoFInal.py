...
def analyze_data(gni_df, internet_df):
    ...
    for country in ["Colombia", "United States", "China"]:
        if country in merged["Country Name"].values:
            df_country = merged[merged["Country Name"] == country]
            plt.figure(figsize=(10, 6))
            plt.plot(years, df_country[[f"{y}_GNI_pc" for y in YEARS]].values.flatten(), 
                     marker="o", label="GNI per capita", color="orange")
            plt.plot(years, df_country[[f"{y}_InternetPct" for y in YEARS]].values.flatten(), 
                     marker="s", label="% Internet Users", color="blue")
            plt.title(f"Evolución GNIpc vs Internet en {country}")
            plt.xlabel("Año")
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"evolucion_gni_internet_{country}.png")
            plt.close()

    # --- 2. Scatter Internet vs GNI per capita ---
    ...
    # --- 3. Tasas de crecimiento comparadas ---
